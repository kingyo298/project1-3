...
class EmotionObject{
  ...
  getOnomatopoeiaWords(){
    ...
    for(let i = 0; i < this.onomatopoeia.length; i++){
      let temp = this.onomatopoeia[i];
      objArray.push(new Word(temp,dictionary[temp],pictureDictionary[temp]));
    }
    return objArray;
  }

  getHtmlContainerString(){
    let objArray = this.getOnomatopoeiaWords();
    let emotionCards = ``;
    for(let i = 0; i < objArray.length; i++){
      emotionCards += `
        <div class="d-flex bg-white col-md-5 col-12 my-2 px-0">
          <div class="col-8">
            <h4 class="pt-3">${objArray[i].word}</h4>
            <p class="pt-2">${objArray[i].definition}</p>
          </div>
          <div class="col-4 px-0 d-flex justify-content-center align-items-center">
            <img class="col-12 p-1" src=${objArray[i].pictureUrl}>
          </div>
        </div>
      `;
    }
    let section = document.createElement("section");
    section.id = `${this.emotion}`;
    section.style = `background-color: ${this.color}`;
    section.innerHTML = `
...
function createEmotionCard(obj){
  let cardItem = document.createElement("a");
  cardItem.classList.add("makeLink", "col-12", "col-md-6", "col-lg-3", "m-4", "p-4", "text-center");
  cardItem.style = `background-color: ${obj.color}`;
  cardItem.href = `#${obj.emotion}`;
  cardItem.innerHTML = `
    <h2 class="text-white">${obj.emotion}</h2>
    <h1>${obj.emoji}</h1>
...
emotions.map(emotion => {
  cards.append(createEmotionCard(emotion));
  sections.append(emotion.getHtmlContainerString());
});
